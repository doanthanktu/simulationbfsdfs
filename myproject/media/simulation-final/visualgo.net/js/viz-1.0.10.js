...
let speedVal = 0.3; function createPlaybackSpeedSlider(defaultPlaybackSpeed) { $("#speed-input").slider({ range: false, min: 0.5, max: 6.5, value: defaultPlaybackSpeed, step: 1.0, change: function (event, ui) { gw.setAnimationDuration(700 / ui.value); $('#viz-speed-value').html(Math.round((speedVal + Number.EPSILON) * 100) / 100 + "x"); }, slide: function (event, ui) { speedVal = ui.value; $('#viz-speed-value').html(Math.round((speedVal + Number.EPSILON) * 100) / 100 + "x"); } }); speedVal = defaultPlaybackSpeed; }
$(function () {
    $("#progress-bar").slider({ range: "min", min: 0, value: 0, slide: function (event, ui) { gw.pause(); gw.jumpToIteration(ui.value, 0); }, stop: function (event, ui) { if (!isPaused) { setTimeout(function () { gw.play(); }, 500); } } }); initUI(); $('#mode-button').click(function () { $('#other-modes').toggle(); }); $('#mode-menu').hover(function () { $('#other-modes').show(); }, function () { $('#other-modes').hide(); }); $('#mode-menu a').hover(function () { $(this).css("background", surpriseColour); }, function () { $(this).css("background", "rgb(0,0,0)"); }); $('#other-modes a').click(function () {
        ...
        else if (event.which == 189) { speedVal = Math.max(0, speedVal - 1); $("#speed-input").slider("value", speedVal); }
        else if (event.which == 187) { speedVal = Math.min(7, speedVal + 1); $("#speed-input").slider("value", speedVal); }
    });
...
function toggleScale() {
    if (isPlaying) stop(); isPlaying = false; if (scale)
